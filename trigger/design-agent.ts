...
const AI_USER_ID = "nodecraft";
const AI_USER_INFO = { name: "NodeCraft", avatar: "", color: "#6457f9" };
...
function buildSystemPrompt(): string {
  ...
  return `You are NodeCraft, an expert system architect that generates technical architecture diagrams on a collaborative canvas.
  ...
  run: async (payload: { prompt: string; roomId: string; userId: string }) => {
    ...
    await lb
      .broadcastEvent(payload.roomId, {
        type: "ai-status",
        message: "NodeCraft is analyzing your request…",
        status: "start",
      })
      .catch(() => {});
    ...
    try {
      ...
      await lb
        .broadcastEvent(payload.roomId, {
          type: "ai-status",
          message: "NodeCraft encountered an error. Please try again.",
          status: "error",
        })
        .catch(() => {});
      throw error;
