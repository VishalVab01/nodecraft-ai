...
export function AiSidebar({ isOpen, onClose, roomId, projectId }: AiSidebarProps) {
  ...
  const handleRunTerminal = useCallback(
    (status: string, output: unknown) => {
      ...
      const content = isSuccess
        ...
        : "NodeCraft encountered an error. Please try again."
  ...
  const handleSend = useCallback(async () => {
    ...
    createFeedMessage(FEED_ID, {
      text: "NodeCraft is analyzing your request…",
      status: "start",
    }).catch(() => {})

    setStatusText("NodeCraft is analyzing your request…")
    ...
    try {
      ...
      createFeedMessage(CHAT_FEED_ID, {
        sender: "NodeCraft",
        role: "assistant",
        content: "Failed to reach NodeCraft. Please try again.",
        timestamp: new Date().toISOString(),
      }).catch(() => {})

      createFeedMessage(FEED_ID, {
        text: "NodeCraft encountered an error.",
        status: "error",
      }).catch(() => {})
    ...
    >
      ...
      <div className="flex shrink-0 items-center gap-3 border-b border-border-default px-5 py-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-text-muted">Collaborate with NodeCraft</p>
        </div>
        {isLoading && (
